...
- **Último trabalho:** **Fluxo front "sem atrito" canonizado (2026-07-10)** — Kickoff Input (`00-Input.md` → `00-DNA.md`, matriz linha 20), padrão de vídeo cinematográfico + mídia animada no `[[Asset Sizing Standard]]` v2.0, **AVIF+WebP vence WEBP** (conflito resolvido), ingestão de assets do cliente (Fase 6.0), SEO técnico (Fase 9) e segurança de front (Fase 10) no `[[Frontend Creative Protocol]]` v2.0, `[[Deploy Protocol]]` v1.0 (linha 21), DX obrigatória (Prettier+Husky+lint-staged). Ver decisão [2026-07-10] abaixo. **Pendência futura declarada pelo dev: criar Back Onboarding Protocol após concluir o front.**
